...
./rdma_plugin/rdma_plugin ${old_init_pid} /dev/shm/restorerdma/
...
brown_time=`echo "scale=3; ( $end - $start ) * 1000 / 1.0" | bc`
...
checkpoint_time=`echo "scale=3; ( $checkpoint_end - $checkpoint_start ) / 1.0" | bc`
...
black_time=`echo "scale=3; ( $end - $start ) * 1000 / 1.0" | bc`

echo "Brownout time: ${brown_time} ms"
echo "Blackout time: ${black_time} ms"
