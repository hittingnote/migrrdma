...
static void *restore_qp(void *parent, int qp_fd,
	...
	tmp_qp = ibv_resume_create_qp_v2(tmp_pd->context, tmp_pd,
...
static void *restore_srq(void *parent, int srq_fd,
	...
	srq = ibv_resume_srq_v2(tmp_pd, &srq_param);
	if(!srq) {
		*p_err = -1;
		return NULL;
	}
...
static void *restore_cq(void *parent, int cq_fd,
	...
	tmp_cq = ibv_resume_cq_v2(tmp_context, &cq_param);
	if(!tmp_cq) {
		*p_err = -1;
		return NULL;
	}
...
static void *restore_context(void *parent, int cmd_fd,
	...
	context = ibv_resume_context_v2(ibv_device_list, &context_param);
	if(!context)
		*p_err = -1;
	else
		*p_err = 0;
...
static int prepare_qp_replay(struct ibv_qp *orig_qp, struct ibv_qp *new_qp) {
	return get_ops(new_qp->context)->prepare_qp_recv_replay_v2(orig_qp, new_qp);
}
