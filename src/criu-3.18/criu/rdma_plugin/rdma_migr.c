...
static int __dump_rdma_info(int img_fd, int rdma_fd, char *fname, size_t size) {
	...
	while(1) {
		...
		this_size = read(fd_in, buf, size > 4096? 4096: size);
		if((int)this_size < 0) {
			err_info("read error. errno: %d\n", errno);
			dbg_info("fd_in: %d, size: %d\n", fd_in, size);
			err = -1;
			break;
		}
		if(!this_size)
		...
		write_size = write(fd_out, buf, this_size);
		if((int)write_size < 0) {
			err_info("write error. errno: %d\n", errno);
			dbg_info("fd_out: %d, this_size: %d\n", fd_out, this_size);
			err = -1;
			break;
		}
