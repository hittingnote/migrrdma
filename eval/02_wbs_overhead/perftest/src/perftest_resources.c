...
int run_iter_bw(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	...
	sockfd = malloc(sizeof(int) * num_of_qps);

	dirfd = open("/proc/self/fd", O_DIRECTORY);
	dir = fdopendir(dirfd);
	while((ent = readdir(dir)) != NULL) {
		...
		ret_size = readlinkat(dirfd, ent->d_name, buf, 4096);
		sprintf(linkname, "%.*s", (int)ret_size, buf);
		if(strncmp(linkname, "socket", strlen("socket"))) {
			continue;
		}

		sockfd[curp++] = atoi(ent->d_name);
	}
